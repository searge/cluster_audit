...
charts/platform-capacity/dashboards/ feeds the deployed ConfigMap. The pair
...
def test_chart_dashboard_matches_grafana_source() -> None:
    ...
    chart_copy = REPO / "charts" / "platform-capacity" / "dashboards" / "capacity.json"

    assert chart_copy.read_bytes() == source.read_bytes(), (
        "charts/platform-capacity/dashboards/capacity.json differs from "
