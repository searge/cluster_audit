...
class K8sResourceAuditor:
    ...
    def collect_cluster_data(self) -> AuditSnapshot:
        ...
        for node in nodes_data['items']:
            ...
            nodes.append(NodeInfo(
                ...
                memory_allocatable=self.parse_memory(allocatable.get('memory', '0Ki')),
    ...
    def calculate_cluster_stats(self, nodes: list[NodeInfo], pods: list[PodInfo]) -> dict[str, Any]:
        """Calculate cluster-wide statistics"""
        total_containers = sum(len(pod.containers) for pod in pods)
        containers_with_issues = sum(1 for pod in pods for container in pod.containers if container.issues)
        ...
        return {
            ...
            'issue_rate': containers_with_issues / total_containers if total_containers > 0 else 0,
            'severity_breakdown': dict(severity_counts),
            'resource_utilization': {
                'cpu_requests_ratio': total_cpu_requests / cluster_cpu_capacity if cluster_cpu_capacity > 0 else 0,
                'cpu_limits_ratio': total_cpu_limits / cluster_cpu_capacity if cluster_cpu_capacity > 0 else 0,
                'memory_requests_ratio': total_memory_requests / cluster_memory_capacity if cluster_memory_capacity > 0 else 0,
                'memory_limits_ratio': total_memory_limits / cluster_memory_capacity if cluster_memory_capacity > 0 else 0,
            }
        }
    ...
    def generate_reports(self, snapshot: AuditSnapshot, previous_snapshots: list[AuditSnapshot]) -> None:
        ...
        for pod in snapshot.pods:
            for container in pod.containers:
                rows.append({
                    ...
                    'node': pod.node,
                    'node_type': next((n.node_type for n in snapshot.nodes if n.name == pod.node), 'unknown'),
    ...
    def generate_nodes_csv(self, snapshot: AuditSnapshot, timestamp: str) -> None:
        ...
        node_usage: dict[str, dict[str, Any]] = defaultdict(lambda: {
            'pod_count': 0, 'cpu_requests': 0.0, 'cpu_limits': 0.0,
            'memory_requests': 0, 'memory_limits': 0, 'issues_count': 0
        })
        ...
        for pod in snapshot.pods:
            if pod.node != 'Unknown':
                ...
                node_usage[pod.node]['issues_count'] += sum(1 for c in pod.containers if c.issues)
        ...
        for node in snapshot.nodes:
            ...
            rows.append({
                ...
                'cpu_request_ratio': f"{usage['cpu_requests'] / node.cpu_allocatable * 100:.1f}%" if node.cpu_allocatable > 0 else "0%",
                'cpu_limit_ratio': f"{usage['cpu_limits'] / node.cpu_allocatable * 100:.1f}%" if node.cpu_allocatable > 0 else "0%",
                'memory_request_ratio': f"{usage['memory_requests'] / node.memory_allocatable * 100:.1f}%" if node.memory_allocatable > 0 else "0%",
                'memory_limit_ratio': f"{usage['memory_limits'] / node.memory_allocatable * 100:.1f}%" if node.memory_allocatable > 0 else "0%",
                'issues_count': usage['issues_count'],
                'pod_utilization': f"{usage['pod_count'] / node.pod_capacity * 100:.1f}%" if node.pod_capacity > 0 else "0%"
            })
    ...
    def generate_namespaces_csv(self, snapshot: AuditSnapshot, timestamp: str) -> None:
        ...
        ns_stats: dict[str, dict[str, Any]] = defaultdict(lambda: {
            'pod_count': 0, 'container_count': 0, 'issues_count': 0,
        ...
        for ns, stats in ns_stats.items():
            health_score = max(0, 100 - (stats['issues_count'] / stats['container_count'] * 100)) if stats['container_count'] > 0 else 100
    ...
    def generate_trends_csv(self, snapshots: list[AuditSnapshot]) -> None:
        ...
        for snapshot in snapshots:
            rows.append({
                ...
                'containers_with_issues': snapshot.cluster_stats['containers_with_issues'],
                ...
                'critical_issues': snapshot.cluster_stats['severity_breakdown'].get('CRITICAL', 0),
                'high_issues': snapshot.cluster_stats['severity_breakdown'].get('HIGH', 0),
                'medium_issues': snapshot.cluster_stats['severity_breakdown'].get('MEDIUM', 0),
                'low_issues': snapshot.cluster_stats['severity_breakdown'].get('LOW', 0)
            })
    ...
    def generate_recommendations_md(self, snapshot: AuditSnapshot, previous_snapshots: list[AuditSnapshot]) -> None:
        """Generate actionable recommendations"""
        filename = self.data_dir / f"recommendations_{snapshot.timestamp.strftime('%Y%m%d_%H%M%S')}.md"

        stats = snapshot.cluster_stats

        with open(filename, 'w') as f:
            f.write("# Kubernetes Resource Audit Report\n")
            f.write(f"**Generated**: {snapshot.timestamp.strftime('%Y-%m-%d %H:%M:%S')}\n\n")
            ...
            f.write(f"| **Containers with Issues** | {stats['containers_with_issues']} ({stats['issue_rate']*100:.1f}%) |\n")
            f.write(f"| **CPU Limits Overcommit** | {stats['resource_utilization']['cpu_limits_ratio']*100:.1f}% |\n")
            f.write(f"| **Memory Limits Overcommit** | {stats['resource_utilization']['memory_limits_ratio']*100:.1f}% |\n\n")
            ...
            if previous_snapshots:
                ...
                issue_trend = stats['containers_with_issues'] - prev.cluster_stats['containers_with_issues']
                trend_emoji = "📈" if issue_trend > 0 else "📉" if issue_trend < 0 else "➡️"

                f.write(f"- **Issues trend**: {trend_emoji} {issue_trend:+d} since last audit\n")
                f.write(f"- **Pod growth**: {stats['total_pods'] - prev.cluster_stats['total_pods']:+d} pods\n\n")

            # Top Priority Actions
            f.write("## 🎯 Priority Actions\n\n")

            if stats['severity_breakdown'].get('CRITICAL', 0) > 0:
                f.write(f"### 🚨 URGENT: {stats['severity_breakdown']['CRITICAL']} containers without any limits\n")
            ...
            if stats['resource_utilization']['cpu_limits_ratio'] > 1.5:
                ...
                f.write(f"Current CPU overcommit ({stats['resource_utilization']['cpu_limits_ratio']*100:.1f}%) will cause issues during migration.\n")
            ...
            f.write("kubectl delete pod --field-selector=status.phase==Completed -A\n\n")
